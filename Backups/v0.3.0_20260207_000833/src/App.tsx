...
function App() {
  ...
  // ========== FOCUS GUARDIAN INITIALIZATION ==========
  // Apply WS_EX_NOACTIVATE style on app startup to prevent focus stealing
  useEffect(() => {
    const initNoActivate = async () => {
      try {
        await invoke('apply_noactivate');
        console.log('✅ Focus Guardian initialized (WS_EX_NOACTIVATE applied)');
      } catch (error) {
        console.error('❌ Failed to apply noactivate style:', error);
      }
    };
    initNoActivate();
  }, []);
